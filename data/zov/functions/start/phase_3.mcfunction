...
# Вызывается через 16 сек после start (8+8)
# или немедленно если пропущена заставка
# ================================================

# Сбрасываем флаг пропуска заставки после использования
scoreboard players set #skip_intro fl_math 0

gamemode survival @a

# Неуязвимость на время подготовки
...
# Инициализация таймера в масштабе боссбара
scoreboard players set #prep_timer fl_math 24000
scoreboard players set #prep_hud_timer fl_math 0

# Если пропуск подготовки включён — ставим 1 тик, prep_end сработает немедленно
