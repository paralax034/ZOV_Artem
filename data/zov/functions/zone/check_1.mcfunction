...
# Порог захвата: 200 (40 сек) — быстрее остальных, A1 вводная
# Шкала 200 обрабатывается внутри общего zone/effects
# tag=!fl_waiting — мёртвые не считаются в зоне
# ================================================

# Считаем присутствие живых игроков обеих команд в зоне
...
# Определяем состояние зоны: 0=пусто/только синие, 1=захват, 2=оспаривается
...
# Прогресс: +1 при захвате, -1 при idle (не ниже 0)
...
# Визуализация: единый файл effects обрабатывает шкалу A1 (200) и A2..7 (300)
function zov:zone/effects

# Порог A1 = 200 (40 сек при захвате без боя)
